...
exports.updateRequest = async (req, res) => {
  try {
    ...
    // Check if vehicle number changed and if there are conflicts
    if (requestData.vehicleNumber !== existingRequest.vehicleNumber) {
      const existingRequests = await Request.findAll({
        where: {
          vehicleNumber: requestData.vehicleNumber,
          status: {
          ...
          id: {
            [require('sequelize').Op.ne]: id // Exclude current request
          }
        },
        order: [['submittedAt', 'DESC']]
      });
    ...
    // Handle existing images from form data (for cases where images are passed as form fields)
    const existingImageKeys = Object.keys(requestData).filter(key => key.startsWith('existingImage_'));
    if (existingImageKeys.length > 0) {
      ...
      for (const key of existingImageKeys) {
        ...
        const imageUrl = requestData[key];
        if (imageUrl) {
          await RequestImage.create({
            requestId: id,
            imagePath: imageUrl,
